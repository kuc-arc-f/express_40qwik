...
import BookMarkShow from './pages/BookMarkShow';
//
...
//MPA
app.get('/bookmark/show', (req: any, res: any) => {
  try { res.send(renderToString(BookMarkShow())); } catch (error) { res.sendStatus(500);}
});
app.get('/bookmark', (req: any, res: any) => {
  try { res.send(renderToString(BookMark())); } catch (error) { res.sendStatus(500);}
});
